...
namespace SkiaSharp.Extended.Encoding
{
	internal class LZWEncoder
	{
		...
		public LZWEncoder(SKQuantizedFrame quantizedFrame)
		{
			pixelArray = quantizedFrame.IndexedPixels;
			initCodeSize = Math.Max(2, quantizedFrame.ColorDepth);
		}

		public void Encode(Stream stream)
		{
			stream.WriteByte(Convert.ToByte(initCodeSize)); // write "initial code size" byte

			// reset navigation variables
			curPixel = 0;

			Compress(initCodeSize + 1, stream); // compress and write the pixel data
		...
		private void Compress(int init_bits, Stream stream)
		{
			int fcode;
			int i /* = 0 */;
			int c;
		...
		private int NextPixel() =>
			(curPixel <= pixelArray.Length - 1)
				? (pixelArray[curPixel++] & 0xff)
				: EOF;
